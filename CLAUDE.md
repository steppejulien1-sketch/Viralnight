...
| `club-app.html` | **L'appli des gerants**, cote club, dans le meme cadre de telephone qu'`app-preview.html`. Connexion, tableau de bord, boutique de recompenses, bareme, QR, reglages, Instagram. Un seul gros fichier, donnees reelles (`dashboardData.js`). Julien : « le truc que tu m'as fait [`app.html`], c'est un site, ce n'est pas fou ». Contient le **parcours d'installation** (refait le 16/09/2026, fond blanc sans bandeau mascotte) : six ecrans qui prennent la main a la premiere ouverture d'un etablissement (type Bar/Boite de nuit/Restaurant + nom, adresse, ville, tel -> `category`, `address` ; photos avec l'apercu de la carte telle que la boutique clubbeur la dessine ; catalogue de recompenses ; bareme ; Instagram ; affiche QR). « Nom, ville et photos » dans Reglages rouvre les etapes 2 et 3. Migration `202609160001_adresse_du_club.sql` (droit UPDATE sur address), puis plus jamais -- `establishments.onboarded_at` s'en souvient, le `localStorage` sert de filet. Rejouable depuis Reglages → « Refaire l'installation ». |
