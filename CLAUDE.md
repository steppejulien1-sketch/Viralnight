...
| `app-preview.html` | Maquette autoportante de l'app mobile "clubbeur" (cote client final : scan QR, points, boutique, carte des clubs). Un seul gros fichier, branche sur les vraies fonctions RPC Supabase (`submit_story`, `redeem_reward`, `checkin_scan`...). **Double vie** : au navigateur c'est la maquette de demonstration ; installee sur l'ecran d'accueil ou dans Capacitor, la classe `.mode-appli` fait tomber l'habillage et l'appli passe en plein ecran. Tester avec `?app=1`. Voir `MOBILE.md`. **Accueil et inscription refaits les 14-15/09/2026 sur le parcours de DUSK** : lancement ou le mot Noctify s'ecrit lettre par lettre, accueil noir plein cadre (trois legendes sur les videos `MEDIAS_ACCUEIL` / `MEDIA_LANCEMENT`, generees par Julien, vides tant qu'il ne les a pas livrees), puis une feuille a une question par ecran (email → boite → code, puis nom → capture → pseudo Instagram → cadeau de bienvenue, seulement ce qui manque au compte). La suite part toujours de `onAuthStateChange` → `poursuivreParcours()`, quelle que soit la porte (code, lien, Google). `window.noctifyParcours(ecran, adresse)` pose la feuille sur un ecran sans envoyer d'e-mail (captures). Test de bout en bout avec compte jetable : `outils/parcours_inscription.cjs`. |
...
- **Plus de mascotte dans l'appli clubbeur depuis le 15/09/2026** (Julien : « ca fait trop gamin
  [...] je ne la veux plus »). Le logo est le mot **Noctify** (classe `.mot` d'`app-preview.html` :
  Archivo 800, point du i en rouge `#de3131`), les icones se regenerent avec `npm run icones`, et
  un etablissement sans photo porte `--badge-noctify` (N blanc + point rouge sur noir). Le rouge de
  l'appli clubbeur est celui du logo, `#de3131`, et son mode sombre est un noir neutre (`#000`).
  `club-app.html` (parcours d'installation) utilise encore les poses ci-dessous.
- `public/mascotte/` — le verre Noctify, l'ancienne mascotte de la marque. Une pose par ecran du
...
- **Le code de connexion de l'appli clubbeur fait 8 chiffres, et l'e-mail ne le montre pas encore**
  (constate le 14/09/2026). `mailer_otp_length` = 8 dans la base clubbeur, d'ou `LONGUEUR_CODE = 8`
  dans `app-preview.html` : les deux bougent ensemble ou plus aucun code ne passe. Les modeles
  « Magic Link » et « Confirm signup » sont ceux par defaut (anglais, lien seul, sans `{{ .Token }}`),
  expediteur « ViralNight ». Tant qu'ils ne sont pas changes, l'ecran « Entre le code » n'a rien a
  saisir : seul le lien marche, et dans l'appli installee sur iPhone il ouvre Safari, pas l'appli.
