...
| `club-app.html` | **L'appli des gerants**, cote club, dans le meme cadre de telephone qu'`app-preview.html`. **TROIS onglets depuis le 19/09/2026** : Activite, Boutique, Reglages. L'onglet Points a ete retire (Julien : « supprime l'onglet point mais par contre indique dans l'onglet boutique combien les points rapportent ») : les deux chiffres imposes (100 la story, 20 le scan, `PV_POINTS`) sont en tete de la Boutique, dans deux tuiles claires. `#vue-bareme` ne reste dans le DOM que comme squelette masque -- `rendreBareme()` et son enregistrement y ecrivent encore. QR, fiche du club et pages legales vivent dans Reglages. **Connexion par CODE A 6 CHIFFRES** (19/09/2026, copie de l'appli clubbeur) : le lien magique seul ne ramenait pas dans l'appli sur telephone. `LONGUEUR_CODE = 6` ici ET `mailer_otp_length` = 6 dans la base B2B -- l'e-mail se pose avec `node outils/emails/appliquer_modeles_club.cjs` (`--lire` pour voir l'etat). Le lien de secours marche toujours, donc connexion.html / inscription.html ne perdent rien. **Reglages repris sur l'appli clubbeur** : blocs titres (.pf-section / .pf-liste / .pf-item), icone au trait, une phrase par ligne. **L'EXEMPLE PASSE AUSSI QUAND LE COMPTE N'A PAS DE CLUB** (19/09/2026, « je vois rien ») : `estDemo()` ne regardait que la session, donc un compte sans etablissement voyait une appli VIDE (tirets, boutique sans recompense, « Aucun etablissement lie a ce compte »). Il repose maintenant sur `sansClubReel()` -- source des donnees + presence d'un etablissement. ⚠️ Consequence : `etablissementCourant()` peut renvoyer le club d'EXEMPLE ; tout ce qui ECRIT (parcours, photos, bareme, handle) passe par **`clubAEcrire()`**, jamais par `etablissementCourant()`. **Telephones dessines** : plus d'heure, de wifi ni de batterie nulle part (les images `barre-etat-*.png` ne sont plus posees, ni ici ni dans app-preview.html), et `.st-tel` est en **9/19.5** (proportions d'un vrai iPhone) -- en 9/17.5 le chrome de la story paraissait deux fois trop gros. La scene du clubbeur garde 9/17.5, sa feuille est deja juste en hauteur. **Demonstration accessible sans compte** : « Voir l'appli avec des chiffres d'exemple » sous les boutons de l'ecran du compte -- `.pa-arrivee .pa-tete` masque la barre du haut a l'etape 1, donc le « Passer » du parcours n'existe pas la, et un gerant sans e-mail restait enferme sur cet ecran. Un seul gros fichier, donnees reelles (`dashboardData.js`). Julien : « le truc que tu m'as fait [`app.html`], c'est un site, ce n'est pas fou ». Contient le **parcours d'installation** (refait le 16/09/2026, fond blanc sans bandeau mascotte) : six ecrans qui prennent la main a la premiere ouverture d'un etablissement (type Bar/Boite de nuit/Restaurant + nom, adresse, ville, tel -> `category`, `address` ; photos avec l'apercu de la carte telle que la boutique clubbeur la dessine ; catalogue de recompenses ; bareme ; Instagram ; affiche QR). Il ne revient plus une fois fini -- `establishments.onboarded_at` s'en souvient, le `localStorage` sert de filet. Rejouable depuis Reglages → « Refaire l'installation ». « Nom, ville et photos » dans Reglages rouvre les etapes 2 et 3. Migration `202609160001_adresse_du_club.sql` (droit UPDATE sur address). |
