...
const SOCIAL = [
  ...
  ["colere", /\b(arnaque|escroc|honte|scandale|avocat|plainte|voleur|nul a chier|merde|putain|fdp|connard|degoute|cretin|cretins|con|conne|debile|idiot|abruti|bouffon|tgl|ta gueule|nuls?|sert a rien|servez a rien|inutile|robot de merde|m avez vole|me volez|vous volez|nous volez|du vol|voleurs?|on se fout de)\b/,
...
export const ACCUSE_TRANSMIS = "C'est noté, j'ai transmis ta demande à l'équipe : quelqu'un te répond ici très vite.";

/**
 * Jamais deux fois la meme reponse d'affilee. Julien, 16/09/2026 : a « MAIS
 * TGL » puis « MAIS QUOI CRETIN », le robot avait redonne trois fois le meme
 * paragraphe. Si la reponse calculee est celle deja donnee, on accuse
 * reception une fois (et on transmet) ; si c'est deja fait, on se tait :
 * Julien est prevenu, et un robot qui radote enerve plus qu'il n'aide.
 * @param {{texte:string, transmettre:boolean}} reponse
 * @param {Array<{auteur:string, message:string}>} messages - du plus recent au plus ancien
 * @returns {{texte:string, transmettre:boolean} | null} null = ne rien envoyer
 */
export function eviterRepetition(reponse, messages) {
  const derniereAuto = (messages || []).find((m) => m.auteur === "ia" || m.auteur === "admin");
  if (!derniereAuto || derniereAuto.auteur !== "ia") return reponse;
  const avant = String(derniereAuto.message || "").replace(/^\[Transmis\]\s*/, "");
  if (avant === ACCUSE_TRANSMIS) return reponse.texte === avant || reponse.transmettre ? null : reponse;
  if (avant === reponse.texte) return { texte: ACCUSE_TRANSMIS, transmettre: true };
  return reponse;
}
