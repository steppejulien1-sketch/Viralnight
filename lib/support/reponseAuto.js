...
function repStory(c, maintenant) {
  const s = (c.stories || [])[0];
  if (!s) {
    return r("Je ne vois aucune story reçue sur ton compte. Pour qu'elle compte, mentionne l'établissement dans ta story Instagram avec le compte indiqué dans l'onglet Story, et laisse-la en ligne jusqu'à la vérification. Si tu l'as bien fait, dis-nous où et quand : " + TRANSMIS, true);
  }
...
function repPoints(c, maintenant) {
  ...
  return r(`Ton solde est de ${pts(c.profil?.points_balance)}.${dernier} Une story rapporte 100 points environ 24 h après, un scan du QR 15 points tout de suite. S'il te manque des points après ce délai, dis-nous où et quand : ${TRANSMIS}`, true);
}
