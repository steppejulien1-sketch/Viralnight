...
export function filsSupport(messages, profils = {}) {
  ...
  for (const m of tries) {
    ...
    // « ia » : la reponse automatique du support (lib/support/reponseAuto.js).
    const auteur = m.auteur === "admin" || m.auteur === "ia" ? m.auteur : "client";
    parUser.get(m.user_id).push({ auteur, message: String(m.message || ""), date: m.created_at });
  }
  const fils = [...parUser].map(([userId, lignes]) => {
    ...
    return {
      ...
      sujet,
      // Reponse automatique seule : rien a faire. Transmise : a Julien de jouer.
      aRepondre: dernier.auteur === "client" || (dernier.auteur === "ia" && dernier.message.startsWith("[Transmis]")),
...
/**
 * Ce que rapportera un contenu si on le valide : le MIROIR de la fonction SQL
 * public.story_points (base clubbeur). C'est la base qui credite ; ce chiffre
 * n'est qu'affiche sur le bouton « Valider ». Garder les deux alignes.
 * @param {string} kind - "story" | "reel" | "tiktok"
 * @param {number|null} vuesVerifiees - vues TikTok lues par l'API, sinon null
 */
export function forfaitContenu(kind, vuesVerifiees = null) {
  const k = String(kind || "").toLowerCase();
  if (k === "story") return 100;
  if (k === "reel") return 60;
  if (k === "tiktok") {
    const vues = Math.max(Number(vuesVerifiees) || 0, 0);
    return 60 + Math.min(Math.floor(vues / 100) * 7, 2000);
  }
  return null;
}

/**
 * Validation automatique (migration clubbeur 0047) : ce qui n'est pas refuse
 * 24 h apres l'envoi est valide. La tache tourne a la 17e minute de chaque
 * heure : l'echeance reelle tombe dans l'heure qui suit ces 24 h.
 */
export const DELAI_AUTO_VALIDATION_MS = 24 * 60 * 60 * 1000;

export function echeanceAutoValidation(envoyeLe) {
  const t = new Date(envoyeLe).getTime();
  return Number.isFinite(t) ? new Date(t + DELAI_AUTO_VALIDATION_MS).toISOString() : null;
}

/** « dans 14 h », « dans 35 min », « dans l'heure » : ce que lit Julien sur la ligne. */
export function libelleEcheance(envoyeLe, maintenant = new Date()) {
  const echeance = echeanceAutoValidation(envoyeLe);
  if (!echeance) return null;
  const reste = new Date(echeance).getTime() - new Date(maintenant).getTime();
  if (reste <= 0) return "Validation automatique dans l'heure";
  const minutes = Math.ceil(reste / 60000);
  if (minutes < 60) return `Validation automatique dans ${minutes} min`;
  return `Validation automatique dans ${Math.floor(minutes / 60)} h`;
}

/** Le seuil a passer a la requete de la tache : envoye avant cet instant. */
export function seuilAutoValidation(maintenant = new Date()) {
  return new Date(new Date(maintenant).getTime() - DELAI_AUTO_VALIDATION_MS).toISOString();
}

export function libelleContenu(kind) {
  const k = String(kind || "").toLowerCase();
  if (k === "story") return "Story Instagram";
  if (k === "reel") return "Reel Instagram";
