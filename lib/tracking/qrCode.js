...
/**
 * Le nombre de modules (les petits carres) du QR, marge comprise.
 *
 * L'affiche dessine quatre carres autour du QR : sans cette mesure, ils
 * tombaient n'importe ou et celui du bas a droite chevauchait le motif
 * d'alignement -- « le quatrieme carre est bugue » (Julien, 18/09/2026).
 * Avec elle, ils ont exactement la taille d'un repere du QR (7 modules) et
 * se posent sur sa grille.
 * @param {string} scanUrl
 * @param {number} [marge] modules blancs autour du QR (celle de renderQrPngDataUrl)
 * @returns {number}
 */
export function qrNombreModules(scanUrl, marge = 2) {
  return QRCode.create(scanUrl, { errorCorrectionLevel: "Q" }).modules.size + marge * 2;
}
