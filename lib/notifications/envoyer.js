...
function preparerVapid() {
  ...
  webpush.setVapidDetails(
    process.env.VAPID_SUBJECT || "mailto:noctify.support@gmail.com",
    publique,
    privee
  );
  vapidPret = true;
...
export async function notifierCadeauDuJour() {
  ...
  /* ⚠️ LE JOUR DU CADEAU VIENT DE LA BASE (jour_cadeau : bascule a 10 h,
     heure de Paris). Avant, on sautait tous ceux qui avaient pris le cadeau
     d'HIER (gift_date >= hier en UTC) : quelqu'un qui le prenait chaque jour
     n'etait donc jamais prevenu du suivant -- exactement ceux qu'on veut
     garder. Repli sur la date de Paris si la fonction ne repond pas. */
  const { data: jourBase } = await clubbeur.rpc("jour_cadeau");
  const jour = jourBase || new Date().toLocaleDateString("en-CA", { timeZone: "Europe/Paris" });
  const [{ data: avecClub }, { data: dejaPris }] = await Promise.all([
    clubbeur.from("point_grants").select("user_id").in("user_id", ids),
    clubbeur.from("daily_gifts").select("user_id").in("user_id", ids).eq("gift_date", jour),
  ]);
...
/**
 * Envoie une notification a TOUS les appareils d'une personne.
 * Sert au support (13/09/2026) : l'admin quand quelqu'un ecrit, le clubbeur
 * quand l'admin repond. Meme nettoyage des abonnements morts que les autres
 * envois. Ne leve jamais : une notification ratee ne doit pas faire echouer
 * l'envoi du message lui-meme.
 *
 * @param {string} userId
 * @param {object} evenement - passe tel quel a construireMessage()
 */
export async function pousserA(userId, evenement) {
  try {
    if (!userId || !preparerVapid()) return { envoyees: 0, raison: "vapid_absent" };
    const message = construireMessage(evenement);
    if (!message) return { envoyees: 0, raison: "rien_a_annoncer" };
