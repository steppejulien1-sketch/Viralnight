// Contenu et regles des notifications push.
//
// Volontairement SANS reseau ni DOM : ce fichier decide quoi ecrire et
// quand jeter un abonnement, rien d'autre. L'envoi lui-meme vit dans
// api/credit-clubbeur.js, et la reception dans public/sw.js.
//
// C'est ce decoupage qui rend la partie interessante testable : on peut
// verifier le texte d'une notification et le tri des abonnements morts
// sans jamais toucher a un serveur de push.

/** Longueur au-dela de laquelle Android tronque le corps avec "...". */
const CORPS_MAX = 120;

function nombreLisible(valeur) {
  const n = Number(valeur);
  if (!Number.isFinite(n)) return null;
  return Math.round(n);
}

function couper(texte, max = CORPS_MAX) {
  const propre = String(texte || "").trim().replace(/\s+/g, " ");
  if (propre.length <= max) return propre;
  // On coupe au dernier espace pour ne pas casser un mot en deux.
  const tronque = propre.slice(0, max - 1);
  const espace = tronque.lastIndexOf(" ");
  return (espace > max * 0.6 ? tronque.slice(0, espace) : tronque) + "…";
}

/**
 * Les raisons proposees a la derniere marche de la suppression de compte.
 * Le client n'envoie que le code : un texte libre deguise en motif ne
 * passe pas. Garder alignee avec MOTIFS_DEPART dans app-preview.html.
 */
export const MOTIFS_DEPART = {
  doublon: "J'ai déjà un autre compte",
  aucun_etablissement: "Pas d'établissement près de chez moi",
  pas_de_story: "Ne veut pas publier de stories",
  plus_utilise: "Ne l'utilise plus",
  autre: "Une autre raison",
};

/** Un code inconnu ou absent (ancienne version de l'appli) reste lisible. */
export function libelleMotifDepart(code) {
  return Object.prototype.hasOwnProperty.call(MOTIFS_DEPART, code) ? MOTIFS_DEPART[code] : "Raison non indiquée";
}

/**
 * Construit le contenu d'une notification a partir d'un evenement metier.
 *
 * Retourne null quand il n'y a rien a annoncer : l'appelant ne doit alors
 * envoyer aucune notification. Mieux vaut ne rien dire que reveiller
 * quelqu'un a 3h du matin pour "0 point".
 *
 * @param {object} evenement
 * @param {string} evenement.type - "story_validee" | "story_refusee" | "cadeau_dispo"
 *   | "support_nouveau" | "support_reponse" | "compte_supprime"
 * @param {number} [evenement.points]
 * @param {string} [evenement.club]
 * @param {string} [evenement.pseudo]  - support_nouveau : qui ecrit
 * @param {string} [evenement.extrait] - support_nouveau : ce qu'il ecrit
 * @returns {{titre: string, corps: string, url: string, tag: string} | null}
 */
export function construireMessage(evenement) {
  const type = String(evenement?.type || "");
  const club = String(evenement?.club || "").trim();
  const points = nombreLisible(evenement?.points);

  if (type === "story_validee") {
    // Zero point valide, ce n'est pas une bonne nouvelle a annoncer : ca
    // ouvre l'appli sur une deception. On se tait.
    if (!points || points <= 0) return null;
    const ou = club ? ` chez ${club}` : "";
    return {
      titre: "Ta story est validée",
      corps: couper(`+${points} point${points > 1 ? "s" : ""}${ou}.`),
      url: "/app-preview.html#boutique",
      // Meme tag = la nouvelle notification REMPLACE la precedente au lieu
      // de s'empiler. Trois stories validees d'affilee ne doivent pas
      // donner trois lignes dans le centre de notifications.
      tag: "points",
    };
  }

  /* Le cadeau du jour est pret. Envoye une fois par jour par le cron du
     matin -- c'est la seule notification que personne n'a "declenchee"
     par une action, donc la seule qui doive vraiment meriter sa place.

     ⚠️ ELLE NE DIT PAS COMBIEN. Le montant depend de la marche atteinte
     (2 a 20) et d'un tirage a 1 %, et il est annonce sur la carte, dans
     l'appli. L'ecrire ici obligerait a le calculer avant l'envoi pour
     CHAQUE personne, et surtout : une notification qui annonce "+2
     points" ne fait ouvrir personne. Celle-ci dit qu'il y a quelque
     chose a prendre, l'appli dit quoi. */
  if (type === "cadeau_dispo") {
    return {
      titre: "Ton cadeau du jour t'attend",
      corps: couper("Ouvre l'appli pour le récupérer."),
      url: "/app-preview.html#boutique",
      // Meme tag que les points : si la personne n'a pas ouvert celle
      // d'hier, celle d'aujourd'hui la remplace au lieu d'empiler.
      tag: "points",
    };
  }

  /* Quelqu'un vient d'ecrire au support : pour l'ADMIN (Julien, 13/09/2026 :
     "un vrai systeme, que je recoive quelque chose quand quelqu'un contacte
     le support"). Le debut du message dans la notification : on sait tout
     de suite si c'est urgent sans ouvrir l'appli. */
  if (type === "support_nouveau") {
    const pseudo = String(evenement?.pseudo || "").trim().replace(/^@/, "");
    const extrait = String(evenement?.extrait || "").trim();
    return {
      titre: "Nouveau message au support",
      corps: couper((pseudo ? `@${pseudo} : ` : "") + (extrait || "Un clubbeur a écrit.")),
      url: "/app-preview.html#profil",
      // Un tag par sujet : deux messages de suite remplacent la notification
      // au lieu d'en empiler deux.
      tag: "support",
    };
  }

  /* L'admin a repondu : pour le clubbeur. Le contenu reste dans l'appli --
     une reponse du support peut parler de son compte. */
  if (type === "support_reponse") {
    return {
      titre: "Le support t'a répondu",
      corps: couper("Ta réponse t'attend dans Aide & Support."),
      url: "/app-preview.html#profil",
      tag: "support",
    };
  }

  // Pour Julien : quelqu'un vient de supprimer son compte, et pourquoi.
  if (type === "compte_supprime") {
    const pseudo = String(evenement?.pseudo || "").trim().replace(/^@/, "");
    return {
      titre: "Un compte supprimé",
      corps: couper((pseudo ? `@${pseudo} : ` : "") + libelleMotifDepart(evenement?.motif)),
      url: "/app-preview.html#profil",
      tag: "depart",
    };
  }

  /* Un ami s'est inscrit avec le lien de parrainage (15/09/2026). Pour le
     PARRAIN : c'est la preuve que le systeme marche, sans elle Julien a cru
     n'avoir rien recu. */
  if (type === "parrainage") {
    if (!points || points <= 0) return null;
    const pseudo = String(evenement?.pseudo || "").trim().replace(/^@/, "");
    return {
      titre: "Ton ami s'est inscrit",
      corps: couper(`${pseudo ? "@" + pseudo : "Un ami"} a rejoint Noctify avec ton lien : +${points} point${points > 1 ? "s" : ""}.`),
      url: "/app-preview.html#boutique",
      tag: "points",
    };
  }

  if (type === "story_refusee") {
    return {
      titre: "Ta story n'a pas été retenue",
      corps: couper(
        club
          ? `${club} n'a pas pu valider ta publication. Ouvre l'appli pour voir pourquoi.`
          : "Ouvre l'appli pour voir pourquoi."
      ),
      url: "/app-preview.html#profil",
      tag: "moderation",
    };
  }

  return null;
}

/**
 * Un service de push repond 404 ou 410 quand l'abonnement n'existe plus
 * (appli desinstallee, navigateur reinitialise, permission retiree).
 *
 * Ces lignes doivent etre supprimees, sinon la table grossit
 * indefiniment et chaque envoi paie le cout d'appeler des endpoints
 * morts. Tout autre code -- y compris 429 et les 5xx -- est temporaire :
 * on garde la ligne.
 *
 * @param {number} statut
 * @returns {boolean}
 */
export function abonnementExpire(statut) {
  return statut === 404 || statut === 410;
}

/**
 * Traduit une ligne de la table push_subscriptions vers la forme
 * attendue par la librairie web-push.
 *
 * Retourne null si la ligne est incomplete : une cle manquante ferait
 * lever web-push en pleine boucle d'envoi, et un seul abonnement abime
 * empecherait tous les suivants de partir.
 */
export function versAbonnementWebPush(ligne) {
  const endpoint = ligne?.endpoint;
  const p256dh = ligne?.p256dh;
  const auth = ligne?.auth;
  if (!endpoint || !p256dh || !auth) return null;
  return { endpoint, keys: { p256dh, auth } };
}
